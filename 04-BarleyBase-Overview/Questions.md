...
- `JSX` is preffered just to make use of React in a more easy and effiecient way. So, anything that is done with JSX can also be done with just plain JavaScript.
...
- `ES6` is not mandatory for `React` but it is highly recommended.
...
- In JSX, comments can be written inside a Js block i.e., {}
...
- Also in order to add styles inside `<></>` we should make it as a `<div></div>` element.
...
function App() {
  return (
    <div className="App">
      <Tool name="Arjun Reddy" tool="Figma"/> // name and tool are props
    </div>
  )
}
```
...
- `Config Driven UI` is a concept, that states how the UI should be rendered based on the configurations of the data application receives from Backend
...
- `Real DOM`
  ...
  - Each branch of the tree ends in a node and each node contains object, that programmers can modify the content of the document using a scripting language like javascript.
  - The changes or the updates to the dom are fast because of its tree-like structure but after changes, the updated element and its children have to be re-rendered to update the application UI.
  - so the re-rendering of the UI make the dom slow since all the UI components needs to be rendered for every dom update.
  - As a result, real dom would render not only those items that receives the update but also remaining un-updated objects as well.
